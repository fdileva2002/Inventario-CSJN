...
export default function OtrosConsumiblesPage() {
  ...
  async function loadConsumables() {
    const params: any = { page, limit: rowsPerPage, type: 'OTRO' };
  ...
  async function handleCreateConsumable() {
    try {
      await api.post('/consumables', {
        ...
        brand: newConsumable.brand || undefined,
      ...
      alert(error?.response?.data?.message || 'Error al crear consumible');
    }
  }

  function resetCreateModal() {
    setNewConsumable({ name: '', brand: '', model: '', minimumStock: '', unitMeasure: 'unidad' });
    setOpenCreate(false);
  }
  ...
  function exportToExcel() {
    ...
    XLSX.utils.book_append_sheet(workbook, worksheet, 'Otros consumibles');
    XLSX.writeFile(workbook, `otros_consumibles_${new Date().toLocaleDateString('es-AR').replace(/\//g, '-')}.xlsx`);
  }

  return (
    <AppLayout>
      <Typography variant="h4" sx={{ mb: 3 }}>Otros consumibles</Typography>
      ...
      <Paper sx={{ p: 2, mb: 3 }}>
        ...
        <Box sx={{ display: 'flex', gap: 2, alignItems: 'center' }}>
          <TextField
            label="Buscar por nombre, marca o modelo"
            value={search}
        ...
        <Table>
          ...
          <TableBody>
            ...
            {consumables.length === 0 && (
              <TableRow>
                <TableCell colSpan={7} align="center">No hay consumibles para mostrar</TableCell>
              </TableRow>
            )}
          </TableBody>
        </Table>
      ...
      <Dialog open={openMovement} onClose={() => setOpenMovement(false)} maxWidth="sm" fullWidth>
        ...
        <DialogContent>
          <Typography sx={{ mb: 2 }}>
            {selectedConsumable ? `${selectedConsumable.name} ${selectedConsumable.model}` : ''}
          </Typography>
          <TextField
            select
      ...
      <Dialog open={openDelete} onClose={() => setOpenDelete(false)} maxWidth="xs" fullWidth>
        ...
        <DialogContent>
          <Typography>
            ...
            <strong>{selectedConsumable?.name} {selectedConsumable?.model}</strong>?
